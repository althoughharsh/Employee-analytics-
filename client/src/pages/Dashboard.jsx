  ...
  const fetchEmployees = async (searchQuery = '') => {
    try {
      ...
      const url = searchQuery 
        ? `${API_URL}/api/employees/search?department=${searchQuery}`
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-green-100">
        ...
        <form onSubmit={handleSearch} className="relative w-full md:w-96">
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-green-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors bg-green-50 focus:bg-white"
            value={search}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {employees.map((emp) => (
            <div key={emp._id} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow border border-green-100 group">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-green-400 to-emerald-500 text-white font-bold flex items-center justify-center text-lg">
                    {emp.name.charAt(0)}
                  </div>
                  <div>
